...
AUTHORIZED_ROLES = [1246977272880037956, 1246977272859332662]
LOG_CHANNEL_ID = 1249235583637651507

# IDs do canal de provas e do canal de correção de provas
canal_prova_aluno_id = 1246977273253462070
canal_corrigir_prova_id = 1246977273253462072

# IDs dos cargos que podem visualizar o canal temporário
cargo_visualizacao_1_id = 1246977272880037956
cargo_visualizacao_2_id = 1246977272859332662

# ID do cargo responsável por realizar a prova
cargo_prova_id = 1246977272859332661

# IDs dos cargos
cargo_visualizacao_1_id = 1246977272880037956
cargo_visualizacao_2_id = 1246977272859332662
cargo1_id = 1246977272867586188
cargo2_id = 1246977272867586186  # Funcionário
cargo_para_remover_id = 1246977272859332661

# IDs dos cargos para promoção
cargo_estagiario_id = 1246977272867586188 # Estagiário
cargo_mecanico_id = 1235035964573880398 # Mecânico
cargo_mecanico_senior_id = 1246977272880037952 # Mecânico Sênior

# ID do cargo de devedor
cargo_devedor_id = 1294143105175519302
cargo_devedor_adv_id = 1246977272859332663

# ID do canal de log/relatório de contratação
canal_log_contratacao_id = 1246977274826461268
# ID do canal de log/relatório de promoção
canal_log_promocao_id = 1246977274297978954

#ID do cargo exonerado
cargo_exonerado_id = 1246977274297978953

# IDs dos cargos que você deseja incluir no ranking
cargos_desejados = [
    1246977272867586188, 1235035964573880398, 1246977272880037952,
    1246977272880037953, 1246977272880037954, 1246977272892756109,
    1246977272892756110
]
...
cargos_permitidos = [1246977272859332662, 1246977272880037956]
...
roles_hierarchy = {
    ...
    1235035964624080989: "👑",
    1246977272892756114: "prefeitura",
    1246977272892756110: "chefe",
    1246977272892756109: "sub chefe",
    1246977272880037954: "gerente",
    1235035964599042090: "sub-gerente",
    1246977272880037952: "mecânico senior",
    1235035964573880398: "mecânico",
    1246977272867586188: "estagiário"
}

# ID do canal onde a hierarquia será postada
canal_hierarquia_id = 1246977273693999219 # Substitua pelo ID do canal real
...
async def has_allowed_role(interaction: discord.Interaction):
    ...
    allowed_roles = [1246977272859332662, 1246977272880037956]
...
async def consultarelat(interaction: discord.Interaction, user: discord.User, data_inicio: str, data_fim: str):
    ...
    # Canal #relat-tunning
    canal_relatorios = bot.get_channel(1246977274083803213)
...
async def carregar_relatorios_antigos(channelmensal):
    global relatorios
    print(f"Carregando relatórios antigos entre {data_inicio} e {data_fim}...")
    async for message in channelmensal.history(after=data_inicio, before=data_fim, limit=None):  # Busca todas as mensagens dentro do período
...
async def exibir_ranking():
    ...
    channelmensal = bot.get_channel(canal_ranking_id)

    if not channelmensal:
    ...
    for role_id in cargos_desejados:
        role = channelmensal.guild.get_role(role_id)
        if role:
    ...
    embed.set_thumbnail(url=channelmensal.guild.icon.url)
    ...
    try:
        if mensagem_ranking:
            ...
            mensagem_ranking = await channelmensal.send(embed=embed)
...
async def on_message(message):
    if message.channelmensal.id == 1246977274083803213:  # Canal #relat-tunning
...
async def on_message_edit(before, after):
    if before.channelmensal.id == 1246977274083803213:  # Canal #relat-tunning
...
async def on_ready():
    ...
    try:
        canal_relatorios = bot.get_channel(1246977274083803213)  # Canal #relat-tunning
        if canal_relatorios:
